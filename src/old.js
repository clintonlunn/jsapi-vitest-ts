require([
  ...
  var highlightSymbol = new SimpleLineSymbol(SimpleLineSymbol.STYLE_SOLID, new Color([96, 96, 96]), 3);
  ...
  query(mapView).on('click', function (response) {
    mouseclickQueryTask(parcelsLayerURL + '/0', response.mapPoint)
      .then(function (response) {
        ...
        $('#trsdiv').html(
          '<b>Township, Range, Section:</b> ' +
            response.features[0].attributes.twn +
            ' ' +
            response.features[0].attributes.rng +
            ' ' +
            response.features[0].attributes.sec,
        );
        $('#sharemap').html(
          '<b>Share parcel:</b> <a target="_blank" href=' +
            'http://hermes.freac.fsu.edu/pli-dataviewer/share?parcel_id=' +
            response.features[0].attributes.parcel_id +
            '>http://hermes.freac.fsu.edu/pli-dataviewer/share?parcel_id=' +
            response.features[0].attributes.parcel_id +
            '</a>',
        );
  ...
  function queryParcelOwners(feature) {
    ...
    task.execute(params).then(function (response) {
      ...
      $('#sharemap').html(
        '<b>Share parcel:</b> <a target="_blank" href=' +
          'http://hermes.freac.fsu.edu/pli-new/?parcel_id=' +
          response.features[0].attributes.parcel_id +
          '>http://hermes.freac.fsu.edu/pli-new/?parcel_id=' +
          response.features[0].attributes.parcel_id +
          '</a>',
  ...
  function querySearch(owner, parcel) {
    ...
    task
      ...
      .then(function (i) {
        ...
        $('#trsdiv').html(
          '<b>Township, Range, Section:</b> ' +
            parcelData[i].attributes.twn +
            ' ' +
            parcelData[i].attributes.rng +
            ' ' +
            parcelData[i].attributes.sec,
        );
        $('#legaldiv').html('<b>Legal Description:</b> ' + parcelData[i].attributes.s_legal);
        $('#sharemap').html(
          '<b>Share parcel:</b> <a target="_blank" href=' +
            'http://hermes.freac.fsu.edu/pli-new/?parcel_id=' +
            parcelData[i].attributes.parcel_id +
            '>http://hermes.freac.fsu.edu/pli-new/?parcel_id=' +
            parcelData[i].attributes.parcel_id +
            '</a>',
        );
  ...
  function indexParcels(e) {
    if (e < parcelData.length || e > 0) {
      ...
      $('#trsdiv').html(
        '<b>Township, Range, Section:</b> ' +
          parcelData[e - 1].attributes.twn +
          ' ' +
          parcelData[e - 1].attributes.rng +
          ' ' +
          parcelData[e - 1].attributes.sec,
      );
      $('#sharemap').html(
        '<b>Share parcel:</b> <a target="_blank" href=' +
          'http://hermes.freac.fsu.edu/pli-new/?parcel_id=' +
          parcelData[e - 1].attributes.parcel_id +
          '>http://hermes.freac.fsu.edu/pli-new/?parcel_id=' +
          parcelData[e - 1].attributes.parcel_id +
          '</a>',
      );
  ...
  mapView.when((parcelsLayer.opacity = 0.5));
